...
void _uart_rx_handle(void)
{
    ...
    while(mini_uart_get_rx_buf_size() >= sizeof(acc_data_t))
    {
        memcpy(&rec_acc_data, mini_uart_get_rx_buf(), sizeof(acc_data_t));
           ...
           (rec_acc_data.sync_marker.d == _exp_marker.d))
        {
            ...
            mini_uart_remove_n_rx_buf(sizeof(acc_data_t));
        }
        else
        {
            mini_uart_remove_n_rx_buf(1); //resync by checking byte-for-byte untill sync-marker has been found
        }
    }
}
...
{
    mini_uart_init(_uart_rx_handle);
}
